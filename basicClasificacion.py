'''
Para correr esto en ubuntu y evitar que salgan errores de lineas amarillas
por falta de instalar torch pandas xd:
...
Rencien ahi instalar:
pip install torch pandas numpy matplotlib scikit-learn

y por ultimo correr:
python basicClasificacion.py



Nota: Cada vez que se vuelva abrir el terminal usar:
source venv/bin/activate
...
class MulticlassClassifier(nn.Module):
    # Se añade hidden3=32 para crear la 4ta capa
    def __init__(self, input_dim: int, num_classes: int, hidden1: int = 128, hidden2: int = 64, hidden3: int = 32):
        ...
        self.class_logits = nn.Linear(hidden3, num_classes)       # Capa 4: Predict class scores
        self.class_log_vars = nn.Linear(hidden3, num_classes)     # Capa 4: Predict log-variance for each class

    def forward(self, x: torch.Tensor):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))                                   # Activación de la nueva capa 3
