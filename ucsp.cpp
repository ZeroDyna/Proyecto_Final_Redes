...
int ucsp_distribute(float** data_in_per_worker, size_t* elements_per_worker,
    ...
    for (int i = 0; i < n_workers; i++) {
        ...
        if (ret != 0) {
            ...
            printf("[C++] Worker %d - primeros 5 pesos recibidos: %.7f %.7f %.7f %.7f %.7f\n",
                   i,
                   data_in_per_worker[i][0],
                   data_in_per_worker[i][1],
                   data_in_per_worker[i][2],
                   data_in_per_worker[i][3],
                   data_in_per_worker[i][4]);
        }
    }
