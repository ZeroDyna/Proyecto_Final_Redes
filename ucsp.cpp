...
#include <string.h>
#include <stdlib.h>
#include <zlib.h>          // crc32() — disponible en Linux con -lz
...
// Arma un datagrama completo de 500 bytes con conversión a Network Byte Order.
void ucsp_make_datagram(UCSPDatagram* dg, uint8_t type,
    ...
    // Conversión a formato de red (Big Endian)
    ...
    // El CRC se calcula sobre la cabecera (ya en formato de red) + payload
    uint8_t crc_input[15 + 479];
    memcpy(crc_input,      dg,          15);
    memcpy(crc_input + 15, dg->payload, 479);
    
    // Guardamos el CRC también en formato de red
    dg->crc32 = htonl(ucsp_crc32(crc_input, 15 + 479));
}

// Verifica el datagrama aplicando conversiones inversas
...
// ─────────────────────────────────────────────────────────────
// SECCIÓN 2: Funciones auxiliares internas (Socket y RDT)
// ─────────────────────────────────────────────────────────────
...
static int send_fragments(int sock, const uint8_t* data, size_t total_bytes,
    ...
    for (uint16_t i = 0; i < total_frags; i++) {
        ...
        if (retries == UCSP_MAX_RETRIES) {
            printf("[ucsp] Frag %d falló después de %d intentos. Abortando.\n", i, UCSP_MAX_RETRIES);
            return -1;
        }
    }
    return 0;
...
static int recv_fragments(int sock, uint8_t* out_buf, size_t total_bytes,
    ...
    while (expected < total_frags) {
        ...
        if (ucsp_validate(&dg) != 0 || ntohs(dg.session_id) != session_id) {
            ...
            UCSPDatagram nack;
            ucsp_make_datagram(&nack, UCSP_TYPE_NACK, expected, session_id, 0, 0, NULL, 0);
            sendto(sock, &nack, UCSP_DATAGRAM_SIZE, 0, (struct sockaddr*)src, src_len);
            continue;
        }
        ...
        UCSPDatagram ack;
        ucsp_make_datagram(&ack, UCSP_TYPE_ACK, ntohl(dg.seq_num), session_id, 0, 0, NULL, 0);
        sendto(sock, &ack, UCSP_DATAGRAM_SIZE, 0, (struct sockaddr*)src, src_len);
...
// ─────────────────────────────────────────────────────────────
// SECCIÓN 3: API pública — Lado Main (quien envía)
// ─────────────────────────────────────────────────────────────
...
int ucsp_send_data(float* data_in, size_t send_elements,
    ...
    while (retries < UCSP_MAX_RETRIES) {
        UCSPDatagram syn;
        ucsp_make_datagram(&syn, UCSP_TYPE_SYN, 0, session_id, 0, 0, NULL, 0);
        sendto(sock, &syn, UCSP_DATAGRAM_SIZE, 0, (struct sockaddr*)&worker_addr, addr_len);
    ...
    int ret = send_fragments(sock, (uint8_t*)data_in, send_bytes, session_id, &worker_addr, addr_len);
    if (ret != 0) { close(sock); return -3; }

    // 3. Recibir Data (Gradientes)
    size_t recv_bytes = recv_elements * sizeof(float);
    ret = recv_fragments(sock, (uint8_t*)data_out, recv_bytes, session_id, &worker_addr, addr_len);
    if (ret != 0) { close(sock); return -4; }

    // 4. FIN
    UCSPDatagram fin;
    ucsp_make_datagram(&fin, UCSP_TYPE_FIN, 0, session_id, 0, 0, NULL, 0);
    sendto(sock, &fin, UCSP_DATAGRAM_SIZE, 0, (struct sockaddr*)&worker_addr, addr_len);
...
int ucsp_distribute(float** data_in_per_worker, size_t* elements_per_worker,
    ...
    for (int i = 0; i < n_workers; i++) {
        ...
        int ret = ucsp_send_data(data_in_per_worker[i], elements_per_worker[i],
                                 workers[i].ip, workers[i].port,
                                 results[i], model_params);
        if (ret != 0) {
            printf("[ucsp] Worker %d falló.\n", i);
            failed++;
        }
    }
    ...
    if (success == 0) {
        for (int i = 0; i < n_workers; i++) delete[] results[i];
        delete[] results;
        return -1;
    }
...
// ─────────────────────────────────────────────────────────────
// SECCIÓN 4: API pública — Lado Worker (quien escucha)
// ─────────────────────────────────────────────────────────────
...
int ucsp_listen(int port, size_t expected_recv_elements, size_t elements_to_return, DataCallback callback) {
    ...
    while (true) {
        ...
        ssize_t n = recvfrom(sock, &dg, UCSP_DATAGRAM_SIZE, 0, (struct sockaddr*)&main_addr, &main_len);
        ...
        ucsp_make_datagram(&syn_ack, UCSP_TYPE_SYN_ACK, 0, session_id, 0, 0, NULL, 0);
        sendto(sock, &syn_ack, UCSP_DATAGRAM_SIZE, 0, (struct sockaddr*)&main_addr, main_len);

        memset(recv_buf, 0, recv_bytes);
        if (recv_fragments(sock, recv_buf, recv_bytes, session_id, &main_addr, main_len) != 0) continue;

        memset(result_buf, 0, return_bytes);
        callback((float*)recv_buf, expected_recv_elements, result_buf);

        if (send_fragments(sock, (uint8_t*)result_buf, return_bytes, session_id, &main_addr, main_len) != 0) continue;

        // Bug Corregido: Validación estricta del tamaño antes del CRC
        UCSPDatagram fin;
        ssize_t n_fin = recvfrom(sock, &fin, UCSP_DATAGRAM_SIZE, 0, NULL, NULL);
        if (n_fin == UCSP_DATAGRAM_SIZE) {
            if (ucsp_validate(&fin) == 0 && fin.type == UCSP_TYPE_FIN) {
                printf("[Worker] FIN recibido. Sesión %d cerrada.\n", session_id);
            }
        }
    }
